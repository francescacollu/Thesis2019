...
\begin{equation*}
    C(k) = p_0 e^{-p_1 k},
\end{equation*}
where the coefficients $p_0$ and $p_1$ are given by the fit and are displayed in fig.~\ref{fig:16sites_FIT_CFBulkCONNvsGamma}. Here, we can see that the correlation between nearest neighbours, represented by the coefficient $p_0$, gets bigger as $\gamma$ grows. Another interesting feature arising from this plot is the trend of the exponential rate $p_1$ with respect to $\gamma$; it has a non-monotonic profile, with a minimum for $\gamma = 3$. The plot is displayed in fig.~\ref{fig:CFBulkCONN_p1vsGamma} of appendix~\ref{appendix_supplemental}. %The coefficient $p_1$ is the rate at which the correlation decreases as the distance between spins grows. The fact that it is a non-monotonic function of $\gamma$ is a signature of the 
