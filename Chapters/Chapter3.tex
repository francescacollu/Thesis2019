...
As regards dynamics, the temporal evolution of MPOs can be simulated using the \emph{time evolution block decimation} (TEBD) method, developed by Vidal in~\cite{PhysRevLett.91.147902}; it is useful, for instance, to efficiently compute time-dependent properties of 1D closed quantum many-body systems as done in~\cite{PhysRevLett.93.040502}. 
Here, we can briefly see how it works in the case of open many-body systems, i.e. in the MPO formalism. First of all, let us consider the~\ref{eqn:mpo_gen}: repeated single-value decomposition (SVD) of the tensor $C_{i_1,\dots, i_N; j_1,\dots, j_N}$ can lead to
\begin{equation*}
\begin{split}
    \rho_{MPO} = \sum_{i_1, \dots ,i_N = 1}^{d}\quad \sum_{j_1, \dots ,j_N = 1}^{d}\quad \sum_{\alpha_1,\dots, \alpha_{N-1} = 1}^\chi &(\Gamma_{1, \alpha_1}^{[1]i_1, j_1}\lambda_{\alpha_1}^{[1]})(\Gamma_{\alpha_1, \alpha_2}^{[2]i_, j_2}\lambda_{\alpha_2}^{[2]}) \dots \\  \times &(\lambda_{\alpha_{N-1}}^{[N-1]}\Gamma_{N, \alpha_{N-1}}^{[N]i_N, j_N}) \ket{i_1, \dots ,i_N}\bra{j_1, \dots ,j_N}.
    \end{split}
\end{equation*}


So, the formal solution of~\ref{eqn:lindblad_eqn} can be written as
\begin{equation}
